...
def benchUncompiled(tokenizer, model, reps, iters, modelName):
    ...
    for i in range(reps):
        for input_string in ["TensorFlow is", "TensorFlow is a", "TFLite is a"]:
            ...
            t = Timer(lambda: model.generate(**tokenized_input, num_beams=2, pad_token_id=50256))
...
def benchCompiled(tokenizer, model, reps, iters, modelName):
    ...
    for i in range(reps):
        for input_string in ["TensorFlow is", "TensorFlow is a", "TFLite is a"]:
            tokenized_input = tokenizer(input_string, return_tensors="tf")
            t = Timer(lambda: xla_generate(**tokenized_input, num_beams=2, pad_token_id=50256))
            print(t.timeit(number=iters)/iters)
            results += str(t.timeit(number=iters)/iters) + "\n"
